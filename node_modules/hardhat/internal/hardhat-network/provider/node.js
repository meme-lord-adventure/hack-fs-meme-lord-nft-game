    ...
    async traceTransaction(hash, config) {
        ...
        return this._runInBlockContext(new ethereumjs_util_1.BN(block.header.number).subn(1), async () => {
            ...
            for (const tx of block.transactions) {
                let txWithCommon;
                if (tx.type === 0) {
                    txWithCommon = new tx_1.Transaction(tx, {
                        common: vm._common,
                    });
                }
                else if (tx.type === 1) {
                    txWithCommon = new tx_1.AccessListEIP2930Transaction(tx, {
                        common: vm._common,
                    });
                }
                else if (tx.type === 2) {
                    txWithCommon = new tx_1.FeeMarketEIP1559Transaction(Object.assign(Object.assign({}, tx), { gasPrice: undefined }), {
                        common: vm._common,
                    });
                }
                else {
